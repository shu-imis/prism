...
from core.constants import ACTION_TYPES as ACTION_TYPE_DEFS
from core.constants import NODE_TYPES as NODE_TYPE_DEFS
...
ACTION_TYPES = frozenset(ACTION_TYPE_DEFS)
...
class SimulationEngine:
    ...
    def _run_simulation(self) -> list[WorldState]:
        ...
        if resume:
            ...
            # 轮次历史从 simulation_rounds 表重建（检查点只存最新一份 last_state）
            rounds = self._load_persisted_rounds()
            if not rounds and resume.get("last_state"):  # 表行缺失时回退检查点快照
    ...
    def _inject_seed_events(self, feed: ActionFeed, cycle: int) -> None:
        """注入种子事件 —— MiroFish initial_posts 式机制。
    ...
    def _build_observation(self, agent: Agent, agents: list[Agent], feed: ActionFeed, state: WorldState) -> str:
        ...
        for record in records:
            ...
            if (
                ...
                and record.action_type != SEED_ACTION
