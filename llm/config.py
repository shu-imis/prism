...
def get_active_vendor_settings(
    state: dict[int, dict[str, str]] | None = None,
) -> VendorSettings | None:
    """当前生效厂商的 VendorSettings；未配置 API Key 时返回 None。"""
    if state is None:
...
def build_llm_client(max_retries: int | None = None) -> LLMClient | None:
    ...
    settings = get_active_vendor_settings()
    if settings is not None:
        return LLMClient(vendors=[settings], max_retries=retries)
    try:
...
def check_vendor(settings: VendorSettings) -> str:
    """用指定配置发一条最小请求验证连通性，失败抛异常。"""
    client = LLMClient(vendors=[settings], max_retries=0)
