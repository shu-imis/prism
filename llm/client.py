...
Transport = Callable[["VendorSettings", Sequence[Message], dict[str, Any]], str]
...
class LLMClient:
    ...
    def __init__(
        ...
        vendors: Sequence[VendorSettings] | None = None,
    ...
    def from_env(
        cls,
        prefer: LLMVendor = LLMVendor.OPENAI,
        ...
        preferred = [LLMVendor.OPENAI, LLMVendor.ANTHROPIC]
        if prefer == LLMVendor.ANTHROPIC:
            preferred.reverse()

        vendors: list[VendorSettings] = []
        for vendor in preferred:
            if vendor == LLMVendor.OPENAI and oai_key:
                vendors.append(
                    VendorSettings(
                        LLMVendor.OPENAI,
            ...
            if vendor == LLMVendor.ANTHROPIC and ant_key:
                vendors.append(
                    VendorSettings(
                        LLMVendor.ANTHROPIC,
    ...
    def _call_sdk(
        ...
        if vendor.vendor == LLMVendor.OPENAI:
            return self._call_openai(vendor, messages, options)
        if vendor.vendor == LLMVendor.ANTHROPIC:
            return self._call_anthropic(vendor, messages, options)
        raise ValueError(f"不支持的 LLM 厂商: {vendor.vendor}")
    ...
    def _call_openai(
        ...
        model_lower = vendor.model.lower()
...
def _extract_balanced_json(text: str) -> Iterator[str]:
