...
def _validate_scenario(data: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "nodes": nodes,
        "initial_inventory": clamp_int(data.get("initial_inventory"), 0, 100, DEFAULT_INITIAL_INVENTORY),
        "baseline_cost": clamp_int(data.get("baseline_cost"), 0, 100, DEFAULT_BASELINE_COST),
        "baseline_service_level": clamp_float(data.get("baseline_service_level"), 0.0, 1.0, DEFAULT_BASELINE_SERVICE_LEVEL),
    }
...
def _validate_agent_config(data: dict[str, Any]) -> dict[str, Any]:
    ...
    for tmpl in AGENT_TEMPLATES:
        ...
        agents_config[str(tmpl["id"])] = {
            ...
            "activity": round(clamp_float(cfg.get("activity"), 0.0, 1.0, tmpl["activity"]), 2),
            "influence": round(clamp_float(cfg.get("influence"), 0.5, 3.0, tmpl["influence"]), 1),
    ...
    for raw in data.get("seed_events", []):
        ...
        seed_events.append({
            "content": content,
            "cycle": clamp_int(raw.get("cycle"), 1, max_cycle, 1),
        })
