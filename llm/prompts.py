...
_AGENT_RESPONSE_RAW = """你正在参与一场供应链决策推演仿真，你的身份是：
...
_SCENARIO_EXTRACTION_RAW = """你是一位供应链建模专家。用户会给你一份或多份供应链相关文档的内容，请从中抽取供应链推演的场景配置。
...
_NODE_ENUM = "|".join(f"{k}({v})" for k, v in NODE_TYPES)
SCENARIO_EXTRACTION_SYSTEM = _SCENARIO_EXTRACTION_RAW.replace("__NODE_ENUM__", _NODE_ENUM)
