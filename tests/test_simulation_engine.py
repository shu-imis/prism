...
class SimulationEngineTests(unittest.TestCase):
    ...
    def test_simulation_engine_all_agent_failure_saves_checkpoint(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            # 首轮全部失败 → 致命错误
            def failing_transport(vendor, messages, options):
    ...
    def test_seed_events_injected_into_observation(self) -> None:
        ...
        # 信息流中 agent_id=0 的种子事件以种子事件格式渲染进观察条目（标注不可回应）
        self.assertTrue(any("种子事件" in prompt and "不可作为回应对象" in prompt for prompt in round_two))
    ...
    def test_reaction_to_validation_and_fallback(self) -> None:
        """reaction_to 只保留合法行为体名：多目标取首个，节点名/种子事件/自身降级 none。"""
        def fake_transport(vendor, messages, options):
            system = messages[0]["content"]
            if "核心制造商" in system:
                reaction = '"种子事件"'              # 种子事件 → none
