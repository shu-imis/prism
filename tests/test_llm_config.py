...
class LLMConfigTests(unittest.TestCase):
    ...
    def test_llm_json_fallback(self) -> None:
        ...
        def fake_transport(vendor, messages, options):
        ...
        client = LLMClient(
            vendors=[
                VendorSettings(LLMVendor.OPENAI, "gpt-5.6-sol", "key"),
                VendorSettings(LLMVendor.ANTHROPIC, "claude", "key"),
            ],
            max_retries=0,
    ...
    def test_llm_custom_compatible_base_url(self) -> None:
        ...
        self.assertEqual(observed[0].vendor, LLMVendor.OPENAI)
    ...
    def test_clamp_int_handles_infinity(self) -> None:
        """clamp_int：Infinity/-Infinity/NaN 回退默认值，不抛 OverflowError。"""
        self.assertEqual(llm_analysis.clamp_int(float("inf"), 0, 100, 50), 50)
        self.assertEqual(llm_analysis.clamp_int(float("-inf"), 0, 100, 50), 50)
        self.assertEqual(llm_analysis.clamp_int(float("nan"), 0, 100, 50), 50)

    def test_llm_retry_permanent_error_skips_to_next_vendor(self) -> None:
    ...
    def test_truncated_response_retried_with_larger_max_tokens(self) -> None:
        ...
        client = LLMClient(
            vendors=[VendorSettings(LLMVendor.OPENAI, "m", "key")],
            max_retries=0,
        )
        seen_max_tokens = []
    ...
    def test_vendor_state_persist_and_reload(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            fake = FakeKeyring()
            state = {
                2: {"key": "kimi-key", "url": "https://api.moonshot.cn/v1", "model": "kimi-k3"},
            }
            with mock.patch.dict(os.environ, {}, clear=True), \
                ...
                self.assertEqual(reloaded[2]["model"], "kimi-k3")

                settings = llm_config.get_active_vendor_settings()
                self.assertIsNotNone(settings)
                self.assertEqual(settings.vendor, LLMVendor.OPENAI)
    ...
    def test_build_llm_client_without_key_returns_none(self) -> None:
        ...
        with mock.patch.dict(os.environ, {}, clear=True), \
                mock.patch.object(llm_config, "_get_keyring", return_value=None):
            self.assertIsNone(llm_config.get_active_vendor_settings())
